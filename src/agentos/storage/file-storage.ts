...
import type { MemoryRecord, RoleTemplate, RuntimeAgent, SessionState } from "../types.js";
...
export class FileAgentOsStorage implements AgentOsStorage {
  ...
  async listMemory(query: {
    ...
    if (query.sessionId) {
      rows = rows.filter((x) => x.sessionId === query.sessionId);
    }
    if (query.layer) {
      rows = rows.filter((x) => x.layer === query.layer);
    }
    if (query.scope) {
      rows = rows.filter((x) => x.scope === query.scope);
    }
